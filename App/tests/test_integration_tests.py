...
def session_scope():
    """Ensure each test runs in a separate transaction and rolls back."""
    ...
    yield  # Run the test with the current transaction
...
def test_create_user(test_client, session_scope):
...
def test_authenticate(test_client, session_scope):
    user = User(username="bob", password=generate_password_hash("bobpass"))
    db.session.add(user)
    db.session.commit()

    assert login("bob","bobpass") is None

def test_get_all_users_json(test_client, session_scope):
    ...
    assert users_json == [{"id": 1, "username": "bob"}, {"id": 2, "username": "rick"}]

def test_create_student(test_client, session_scope):
...
def test_create_report(test_client, session_scope):
    student = Student(student_id=1, firstname="Bob", lastname="Francis")
    staff = Staff(username="bob", password="bobpass", department="DCIT", faculty="FST")
    db.session.add_all([student, staff])
    db.session.commit()

    report = Report(student_id=student.id, staff_id=staff.id, review="Good", rating=5)
    db.session.add(report)
    db.session.commit()

    report_in_db = db.session.query(Report).filter_by(student_id=student.id).first()
    assert report_in_db is not None
    assert report_in_db.review == "Good"
    assert report_in_db.rating == 5

def test_get_student_reports(test_client, session_scope):
    student = Student(student_id=1, firstname="Bob", lastname="Francis")
    staff = Staff(username="bob", password="bobpass", department="DCIT", faculty="FST")
    db.session.add_all([student, staff])
    db.session.commit()

    report = Report(student_id=student.id, staff_id=staff.id, review="Excellent", rating=4)
    db.session.add(report)
    db.session.commit()

    reports = db.session.query(Report).filter_by(student_id=student.id).all()
    assert len(reports) == 1
    assert reports[0].review == "Excellent"

def test_update_user(test_client, session_scope):
    user = User(username="bob", password="bobpass")
    db.session.add(user)
    db.session.commit()
...
def test_update_report(test_client, session_scope):
    student = Student(student_id=1, firstname="Bob", lastname="Francis")
    staff = Staff(username="bob", password="bobpass", department="DCIT", faculty="FST")
    db.session.add_all([student, staff])
    db.session.commit()
    ...
    report_to_update = db.session.query(Report).filter_by(student_id=student.id).first()
    report_to_update.review = "Excellent"
    report_to_update.rating = 4
    db.session.commit()

    updated_report = db.session.query(Report).filter_by(student_id=student.id).first()
    assert updated_report.review == "Excellent"
    assert updated_report.rating == 4

def test_delete_student(test_client, session_scope):
    student = Student(student_id=1, firstname="Bob", lastname="Francis")
    db.session.add(student)
    db.session.commit()
...
def test_delete_report(test_client, session_scope):
    student = Student(student_id=1, firstname="Bob", lastname="Francis")
    staff = Staff(username="bob", password="bobpass", department="DCIT", faculty="FST")
    db.session.add_all([student, staff])
    db.session.commit()
    ...
    report_to_delete = db.session.query(Report).filter_by(student_id=student.id).first()
    db.session.delete(report_to_delete)
    db.session.commit()

    deleted_report = db.session.query(Report).filter_by(student_id=student.id).first()
